...
inline void ThreeExpoGausFitScintUtilsCh::Fit(TH1 & h){
	...
	double max = h.GetMaximum();
        if(_init_ped<0)_init_ped=-_init_ped;
	...
	Init();cout << reject <<  " reject value " << endl;
        gStyle->SetOptFit(1112); cout <<"Fit Range: " << x_max-0.5e-6 << ", " << x_max+slowrange << endl;
...
//        _func->FixParameter(6,_func->GetParameter(6));
	...
	_fit_res = h.Fit(_func, "LMESN","",x_max+1.5e-6, x_max+slowrange);
        ...
        SetLimits(); reject = kTRUE; cout << reject <<  " reject value changed " << endl; //lets_pause();
	...
	_fit_res = h.Fit(_func, "LM","",x_max-.5e-6, x_max+slowrange);
        ...
        h.Draw("HIST");h.GetYaxis()->SetRangeUser(1e-5,2);_func->Draw("SAME");//lets_pause();lets_pause();lets_pause();lets_pause();lets_pause();
