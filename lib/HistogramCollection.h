...
namespace ana
{
  ...
  public:
    HistogramCollection_t(){ hist.clear(); histname.clear();}

    void Add(TH1* h, int r, int oc, string sn, double volt, string m, int firstt, string o)
    //~ void Add(TH1* h, string T, int oc, int fold, int wl, int run)
    {
       histname.push_back(new ana::HistogramName_t(r, oc, sn, volt, m, firstt, o));
       //~ histname.push_back(new ana::HistogramName_t(T, oc, fold, wl, run));
    ...
    void GetFromFile(string f)
    {
      ...
      while ((key = (TKey*)next()))
      {
        ...
        //TCanvas *c = new TCanvas("c");gPad->SetLogy(); gStyle->SetOptFit(1);
        hist.push_back(h);
    ...
    void DumpToFile(string f, string fileoption="RECREATE")
    {
      ...
      for(auto th: hist){th->Write();}
      dump->Close(); std::cout << "Dumped to: " << fileout << std::endl;

    }
    std::vector<TH1*> GetByOpChannel(int pm)
    {  
       std::vector<TH1*> v;
       for (int i=0; i<hist.size(); i++) { if (histname[i]->GetOpChannel()==pm) { v.push_back(hist[i]);}}
       return v;
    }
    //~ std::vector<TH1*> GetByPMTSN(string SN)
    //~ {
       //~ std::vector<TH1*> v;
       //~ for (int i=0; i<hist.size(); i++) { if (histname[i]->GetPMTSN()==SN) { v.push_back(hist[i]);}}
       //~ return v;
    //~ }
    std::vector<TH1*> GetByRun( int r)
    {
       std::vector<TH1*> v;
       for (int i=0; i<hist.size(); i++) { if (histname[i]->GetRun()==r) { v.push_back(hist[i]);}}
       return v;
    }
    std::vector<TH1*> GetByOpChannel64()
    {  
       std::vector<TH1*> v; v.resize(64);
       for (int i=0; i<hist.size(); i++) { v[histname[i]->GetOpChannel()]=hist[i];}
       return v;
    }
    ...
    {
      TCanvas *c = new TCanvas("c"); float txtsize=0.06;
      ...
      for(auto h: hist)
      {
        ...
        gPad->SetGridx();
        gPad->SetGridy();h->SetLineColor(2);
    ...
    void PlotAll(int logoption=0)
    {
      ...
      for(auto h: hist)
      {
        ...
        if(logoption==1 || logoption==3)  gPad->SetLogx();
        gPad->SetGridx();
        gPad->SetGridy();h->SetLineColor(i+1);i++;
        h->GetXaxis()->SetLabelSize(txtsize);
        h->GetXaxis()->SetNdivisions(5);
        h->GetYaxis()->SetLabelSize(txtsize);
        h->GetXaxis()->SetTitleSize(txtsize);
