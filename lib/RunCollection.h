...
class RunCollection_t
{
    ...
    {
       ifstream ifs(file);
       int r, t; string th, b, d;
       getline(ifs,th); cout << th << endl;
       while(1)
       {
  ...
  std::vector<std::pair<int, TDatime>> GetByTriggerThreshold(int tr, string th)
  {
    ...
    for(auto l : list)
    {
       if(l.trigger==tr && l.threshold==th) v.push_back(std::pair<int,TDatime>({l.run,l.date}));
    }
    return v;
  }
  std::vector<std::pair<int, string>> GetByTriggerDate(int tr, TDatime d)
  {
    ...
    for(auto l : list)
    {
       if(l.trigger==tr && l.date==d) v.push_back(std::pair<int,string>({l.run,l.threshold}));
    }
    return v;
  }
  std::vector<std::pair<int, int>> GetByThresholdDate(string th, TDatime d)
  {
    ...
    for(auto l : list)
    {
       if(l.threshold==th && l.date==d) v.push_back(std::pair<int,int>({l.run,l.trigger}));
    }
    return v;
  }
};
