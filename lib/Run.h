...
namespace ana
{
   ...
   public:
      ...
      Run_t(int r, std::vector<std::pair<string, string>> ChannelsToRead, string adcm = "", double r1 = 1.e-6, double r2 = 2.e-6, int pr = 30, int mevtp = 200, double conv_factor = 50, string cc = "Gains/gains_20190912.csv", int tr = 250) : Number(r), adcmode(adcm), range1(r1), range2(r2), PedRange(pr), TriggerRange(tr), MaximumEventsToProcess(mevtp), calibration(cc)
      {
         ...
         if (adcmode == "DT5725")
         {
            ...
            ParSet->setConversionFactor(-(16384.0/2.0)*250);
            // ParSet->setConversionFactor(-(16384.0/2.0));
      ...
      void DrawTH2(){
	     ...
	     //  TH2D *h=TH2getWaveform (0, ch, getWaveform(0,ch)->getPedestalMean());
	      // TH2D *h = new TH2D(Form(""),Form(""),getWaveform(0,0)->getNSamples(),0,getWaveform(0,0)->getNSamples()*getWaveform(0,0)->getSampling(),ADCDynamicRange * 10, 0.5, ADCDynamicRange + 0.5);
	      TH2D *h = new TH2D(Form(""),Form(""),250,0,250*4e-9,ADCDynamicRange * 10, 0.5, ADCDynamicRange + 0.5);
	   	...
	   	gPad->SetLogz();
	   	h->GetXaxis()->SetTitle("Time (s)"); h->GetYaxis()->SetTitle("Amplitude");
      ...
      void SetCutIsPedestalRange()
      {
         cout << "CUT: Removing any WAVEFORM with signal in range" << endl; if(EventList.size()==0) Process();      
         for(unsigned int ev=0; ev<EventList.size();ev++) for(auto pm: adcchannels) 
           // {if (getWaveform(ev,pm)->getMaxAmplitudeRange()>(getWaveform(ev,pm)->getPedestalMean()+getWaveform(ev,pm)->getPedestalSTD())) {getWaveform(ev,pm)->setCut(true);} }
           {if (!getWaveform(ev,pm)->getIsPedestalRange()) getWaveform(ev,pm)->setCut(true);}
         MyCuts.SetCutIsPedestalRange();
         CutStatus();
      }
      void SetCutIsPedestal()
      {
         cout << "CUT: Removing any WAVEFORM with signal" << endl; if(EventList.size()==0) Process();      
         for(unsigned int ev=0; ev<EventList.size();ev++) for(auto pm: adcchannels) 
           // {if (getWaveform(ev,pm)->getMaxAmplitudeRange()>(getWaveform(ev,pm)->getPedestalMean()+getWaveform(ev,pm)->getPedestalSTD())) {getWaveform(ev,pm)->setCut(true);} }
           {if ((getWaveform(ev,pm)->getPedestalStatus()) && (!getWaveform(ev,pm)->getIsPedestal())) getWaveform(ev,pm)->setCut(true);}
         MyCuts.SetCutIsPedestal();
         CutStatus();
      }
      void SetCutPeakTimeRangePro(double minTime=0, double maxTime=0, double speAmp=1)
      {
         if(minTime==0 && maxTime==0){minTime=range1; maxTime=range2;}
         ...
         for(unsigned int ev=0; ev<EventList.size();ev++) for(auto pm: adcchannels)
            {if ((getWaveform(ev,pm)->getMaxAmplitudeRange()>speAmp) && (getWaveform(ev,pm)->getPeakTime()<minTime || getWaveform(ev,pm)->getPeakTime()>maxTime)) getWaveform(ev,pm)->setCut(true);}
         MyCuts.SetCutPeakTimeRangePro(minTime,maxTime,speAmp);
         CutStatus();
      }
      void SetCutMaxAmpChargeRange(double maxamp = 0, double maxcharge = 0)
      {
         if(EventList.size()==0) Process();
         cout << "CUT: Selecting WAVEFORMS with Amp < " << maxamp << "ADCs and ChargeRange >" << maxcharge << " on range (" << range1 << "s," << range2 << "s)" << endl;
         for(unsigned int ev=0; ev<EventList.size();ev++) for(auto pm: adcchannels)
            {if ((getWaveform(ev,pm)->getMaxAmplitudeRange()<maxamp) && (getWaveform(ev,pm)->getChargeRange()>maxcharge)) getWaveform(ev,pm)->setCut(true);}
         MyCuts.SetCutMaxAmpChargeRange(maxamp,maxcharge);
      }
      ...
      */
      ...
      TH2D *TH2VariableVsTime(string var, int pm)
      {
         ...
         // TH2D *th = new TH2D(Form("Run%i_OpChannel%i_%s_%.0fV_TH2_%sVsTime", Number, pm, PMT_SN[pm].c_str(), PMT_Voltages[pm], var.c_str()), Form(";Time;%s", var.c_str()), 1000, 0, 1e-6, ADCDynamicRange, 0, ADCDynamicRange);
         if ("MaxAmplitudeRange")
            for (unsigned int ev = 0; ev < NEventsALL; ev++)
            {
               ...
               // th->Fill(wv.getPeakTimeRange(), wv.getMaxAmplitudeRange());
      ...
      {
         TH2F *th = new TH2F(Form("Run%i_OpChannel%i_PedestalVsPedSTD", Number, pm), Form("Run%i_OpChannel%i_PedestalVsPedSTD;PedSTD (ADC);Pedestal (ADC)", Number, pm), 200, 0, 20, ADCDynamicRange, 0, ADCDynamicRange);
         th->SetLineColor(pm + 1);
         for (unsigned int ev = 0; ev < NEvents; ev++)
            if (!getWaveform(ev, pm)->getCut())
            {
      ...
      TH1F *TH1Amp2(int pm)
      {
         TH1F *th = new TH1F(Form("Run%i_OpChannel%i_Amp", Number, pm), Form("Run%i_OpChannel%i_%s_Amp;Amplitude (ADC);Events", Number, pm, PMT_SN[pm].c_str()), ADCDynamicRange*2, 0.5, ADCDynamicRange + 0.5);
         th->SetLineColor(pm + 1);
         for (unsigned int ev = 0; ev < NEvents; ev++)
         {
            if (EventList[ev].getCut())
      ...
      TH2F *TH2AmpVsCharge(int pm, string option = "", string units = "PE")
      {
         ...
         TH2F *th = new TH2F(Form("Run%i_OpChannel%i_AmpVsCharge_%s", Number, pm, option.c_str()), Form("Run%i_OpChannel%i_%s_%.0fV_AmpVsCharge_%s;Charge (%s);Amplitude (ADC)", Number, pm, PMT_SN[pm].c_str(), PMT_Voltages[pm], option.c_str(), units.c_str()), 300, yq2[0] - 0.15 * (yq2[1] - yq2[0]), yq2[1] + 0.15 * (yq2[1] - yq2[0]), 8000, 0, ADCDynamicRange);
         // TH2F *th = new TH2F(Form("Run%i_OpChannel%i_AmpVsCharge_%s", Number, pm, option.c_str()), Form("Run%i_OpChannel%i_%s_%.0fV_AmpVsCharge_%s;Charge (%s);Amplitude (ADC)", Number, pm, PMT_SN[pm].c_str(), PMT_Voltages[pm], option.c_str(), units.c_str()), 300, yq2[0] - 0.15 * (yq2[1] - yq2[0]), yq2[1] + 0.15 * (yq2[1] - yq2[0]), 100, 0, 50);
         th->SetLineColor(pm + 1);
         for (unsigned int ev = 0; ev < NEvents; ev++)
         {
            //           double correction=(PedestalPeak[pm]-EventList[pm][i].getPedestalMean())*(2.0/ADCDynamicRange)*Sampling*NSamples/50;
            ...
            if (option == "Range")
               ...
               // th->Fill(getWaveform(ev, pm)->getChargeRange() * ChargeFactor, getWaveform(ev, pm)->getAverageAmplitudeRange());
            if (option == "MaxPeak")
         ...
         th->Draw();
         // th->Draw("COLZ"); gStyle->SetOptStat(0); gPad->Update(); 
         lets_pause();
         delete thaux;
         return th;
      }
 ...
 */
      ...
      */
      ...
      void DumpVariableToNtuple(string filename, bool debug = false)
      {
         ...
         ofile->cd();
         TNtuple *nt = new TNtuple("ntuple", "ntuple", Form("evt:ch:time:Amp:Q1:Q2:Q3:TStartQpeak:TEndQPeak:TDiff"));
         TNtuple *nt_Q = new TNtuple("charge", "charge", "QFixRange1:QFixRange2:QFixRange3:QFixRange4:QPeakRange1:QPeakRange2:QPeakRange3:QPeakRange4:QPeak:QTotal");
         ...
         for (unsigned int ev = 0; ev < NEvents; ev++)
         {
            ...
            for (auto pm : adcchannels)
            {
               ...
               nt_Q->Fill(
                  ...
                  getWaveform(ev, pm)->getCharge()*1e12);
               if (debug)
               {
      ...
      { // Función para realizar ajustes automaticos
         ...
         if (print == true)
         {
            ...
            for (int p=0; p<npeaks;p++)
            {
               ...
               FILE *f = fopen(Form("%s.txt",filename.c_str()), "a");
               if (f == NULL)
               {
