...
{
  std::vector<double> a;
  for(int i=1; i<hh->GetSize();i++){ a.push_back(hh->GetBinContent(i));}
  return a;
}
double VectorDistance(double ped, std::vector<double>pedestales)
{
   double sum=0;
   for(int i=0; i<pedestales.size(); i++) sum+=TMath::Abs(ped-pedestales[i]);
   return sum;
}
...
void printpoints(TGraphErrors *tg)
{
	...
	{
		double x,y,ye;
		tg->GetPoint(i,x,y);	ye=tg->GetErrorY(i);
...
{
	if(val2.value==0 || val1.value==0) return "";
...
string printgain(physvalue val)
{
	...
	if(val.error>19e6) {return Form("%.0f0\u00b1%.0f0",1e-7*val.value,1e-7*val.error);}
	if(val.error>1.9e6) {return Form("%.0f\u00b1%.0f",1e-6*val.value,1e-6*val.error);}
	return Form("%.1f\u00b1%.1f",1e-6*val.value,1e-6*val.error);
}
string printgain2(physvalue val)
{
	if(val.value==0) return "";
	if(val.error>1.9) return Form("%.0f+%.0f",1e-6*val.value,1e-6*val.error);
	return Form("%.1f+%.1f",1e-6*val.value,1e-6*val.error);
}
...
void plotdiscrepancies(std::vector<TH1D *> h,std::vector<TH1D *> hh,TGraphErrors *tg, std::vector<TGraphErrors *> tg2)
{
	...
	for (int i=0; i<tg->GetN();i++)
	{
		tg->GetPoint(i,x,y); ye=tg->GetErrorY(i); map1[(int)x].value=y;map1[(int)x].error=ye;
	}
	for (int k=0; k<tg2.size(); k++) for (int i=0; i<tg2[k]->GetN();i++)
	{
		tg2[k]->GetPoint(i,x,y); ye=tg2[k]->GetErrorY(i); map2[k][(int)x].value=y;map2[k][(int)x].error=ye;
	}

	for (int k=0; k<tg2.size(); k++) for (std::map<int,physvalue>::iterator it=map2[k].begin(); it!=map2[k].end(); ++it)
	{
	  //std::cout << it->first << " => " << it->second.value << '\n';
	  physvalue empty={0,0};
	  std::map<int,physvalue>::iterator it1;
	  it1 = map1.find(it->first);
	  if (it1 == map1.end()) map1.insert (std::pair<int,physvalue>(it->first,empty));
	}

	  cout << Form("Voltage\tGain_%s(10^6)",tg->GetName());
	for (int k=0; k<tg2.size(); k++) cout << Form("\t Gain_%s(10^6)\t\u0394(\u03C3)",tg2[k]->GetName());
	cout << endl;

	for (std::map<int,physvalue>::iterator it=map1.begin(); it!=map1.end(); ++it)
	{
	  
	  for (int k=0; k<tg2.size(); k++) if(it->second.value!=0 && map2[k][it->first].value!=0) h[k]->Fill(getdiscrep(it->second,map2[k][it->first]));

	  for (int k=0; k<tg2.size(); k++) if(it->second.value!=0 && map2[k][it->first].value!=0) hh[k]->Fill(100.0*map2[k][it->first].value/it->second.value - 100);
	  std::cout << it->first << "\t" << printgain(it->second);
	  for (int k=0; k<tg2.size(); k++) std::cout << "\t" << printgain(map2[k][it->first])  << "\t" << discrep(it->second,map2[k][it->first]);
	  cout << endl;
	}
...
for (int i=0;i<mg->GetListOfGraphs()->GetSize();i++)if (((TGraphErrors *)mg->GetListOfGraphs()->At(i))->GetY()[0]<ylow) ylow=((TGraphErrors *)mg->GetListOfGraphs()->At(i))->GetY()[0];
for (int i=0;i<mg->GetListOfGraphs()->GetSize();i++)if (((TGraphErrors *)mg->GetListOfGraphs()->At(i))->GetY()[((TGraphErrors *)mg->GetListOfGraphs()->At(i))->GetN()-1]>yup) yup=((TGraphErrors *)mg->GetListOfGraphs()->At(i))->GetY()[((TGraphErrors *)mg->GetListOfGraphs()->At(i))->GetN()-1];

ylow=0.8*ylow;
yup=2*yup; std::cout << "setlogscale::Setting Ylevels: " << ylow << " " << yup << std::endl;
...
for (int i=0;i<mg->GetListOfGraphs()->GetSize();i++)if (((TGraphErrors *)mg->GetListOfGraphs()->At(i))->GetX()[0]<xlow) xlow=((TGraphErrors *)mg->GetListOfGraphs()->At(i))->GetX()[0];
for (int i=0;i<mg->GetListOfGraphs()->GetSize();i++)if (((TGraphErrors *)mg->GetListOfGraphs()->At(i))->GetX()[((TGraphErrors *)mg->GetListOfGraphs()->At(i))->GetN()-1]>xup) xup=((TGraphErrors *)mg->GetListOfGraphs()->At(i))->GetX()[((TGraphErrors *)mg->GetListOfGraphs()->At(i))->GetN()-1];
	xlow=xlow-LabelStep;
	xup=xup+LabelStep;
	//c1->SetGrid();
	c1->SetLogy();
...
for (int i=0;i<mg->GetListOfGraphs()->GetSize();i++) for (int j=0;j<((TGraphErrors *)mg->GetListOfGraphs()->At(i))->GetN();j++)
if ( ((TGraphErrors *)mg->GetListOfGraphs()->At(i))->GetY()[j]<ylow) ylow=((TGraphErrors *)mg->GetListOfGraphs()->At(i))->GetY()[j];
for (int i=0;i<mg->GetListOfGraphs()->GetSize();i++) for (int j=0;j<((TGraphErrors *)mg->GetListOfGraphs()->At(i))->GetN();j++)
if ( ((TGraphErrors *)mg->GetListOfGraphs()->At(i))->GetY()[j]>yup) yup=((TGraphErrors *)mg->GetListOfGraphs()->At(i))->GetY()[j];
...
for(int i=0;i<mg->GetN();i++) if (mg->GetY()[i]<ylow) ylow=mg->GetY()[i];
for(int i=0;i<mg->GetN();i++) if (mg->GetY()[i]>yup) yup=mg->GetY()[i];
...
xlow=mg->GetX()[0];
xup=mg->GetX()[0];
for(int i=0;i<mg->GetN();i++) if (mg->GetX()[i]<xlow) xlow=mg->GetX()[i];
for(int i=0;i<mg->GetN();i++) if (mg->GetX()[i]>xup) xup=mg->GetX()[i];
...
//Lines needed to have the value in Log Scale
  xlow   = 1000;
  xup    = 1800;
  Int_t    nbinsx = Int_t(xup - xlow); cout << "nbinsx " << nbinsx << endl;
//cout << " ho visto xmin" << endl;
  Int_t    step_normal     = LabelStep;
  Int_t    factor_large    = 1;
  //  Int_t    factor_large    = ;
...
//----------------------------------------------------------------------------
  if (c1->GetLogx()) { cout << "in X " << endl;
    ...
    tick.DrawLine(xx, ymin, xx, ymin + ((i-ixlow+50)%step_large == 0 ? bottom_tick_large : bottom_tick_normal)); cout << "Draw line " << endl;
	...
	for (int i=0;i<_graph.size();i++)
	{
	   ...
	   _graph[i]->SetFillStyle(0);
           ...
           cout << "Creo myfit " << i << endl;
            ...
            {
               if(rangemin>x[j]) rangemin=x[j];
               if(rangemax<x[j]) rangemax=x[j];
            }
	   ...
	   cout << "Ajusto! " << endl;
		...
		{
			_fit_successful[i]=true;
			covarianzaAB.push_back(0.0);cout << "ERROR!!!!" <<endl;lets_pause();
		}
		//lets_pause();
...
{  cout << "Holi" << endl;
   ...
   while ((key = (TKey*)next()))
   {
      ...
      if(histname.find(name)!=std::string::npos){ cout << histname << " FOUND " << name << endl;return h;}
  }
     return NULL;
}
