...
public:
  ...
  bool LoadBinary(unsigned int len, int EN, ULong64_t triggtstmp, vector<int> adc)
  {
    Length = len;
    EventNumber = EN;
    ...
    for (auto amp : adc)
      ADCs.push_back(amp);
      // ADCs.push_back(-amp); // Para leer la SC por su polaridad positiva
    return true;
  }
...
int ReadAndDumpBinary(string file, string outfile)
{
  ...
  while (1)
  {
    ...
    if (wv.size() % 10000 == 0)
    {
      ...
      // break; // just if we want to read a fixed number of waveforms
    }
  }
  fclose(pFile);
