...
class Waveform_t 
{
  ...
  bool LoadBinary(int len,int EN, ULong64_t triggtstmp, vector<int> adc)
  {
    ...
    for (auto amp:adc) ADCs.push_back(amp);
    return true;
  }
  bool Load(ifstream *ifs, bool DEBUG=false, bool TS=false)
  {
    string s;
    getline(*ifs,s); if(DEBUG) cout << s << endl; //Record Length: 3072
    if (ifs->eof()) return false;
    s=s.substr(15,s.length()-15); Length=stoi(s);  if(DEBUG) cout << "Length: " << Length << endl;//lets_pause();
    getline(*ifs,s); if(DEBUG) cout << s << endl;//BoardID:  0
    getline(*ifs,s); if(DEBUG) cout << s << endl;//Channel: 0
    getline(*ifs,s); if(DEBUG) cout << s << endl;//Event Number: 57776
    s=s.substr(14,s.length()-14); EventNumber=stoi(s); if(DEBUG) cout << "EventNumber: "<< EventNumber << endl;
    getline(*ifs,s); if(DEBUG) cout << s << endl;//Pattern: 0x0000
    getline(*ifs,s);if(DEBUG){ cout << s << endl;}//Trigger Time Stamp: 3004795797  2756456643
    s=s.substr(20,s.length()-20); TriggerTimeStamp=stol(s); if(DEBUG) cout << "TriggerTimeStamp: "<< TriggerTimeStamp << endl;
    getline(*ifs,s); if(DEBUG) cout << s << endl;//DC offset (DAC): 0x0CCC
    if(TS)
    {
      getline(*ifs,s); s=s.substr(14,s.length()-14); PCTimeStamp=stol(s); if(DEBUG) cout << "PC TriggerTimeStamp: "<< PCTimeStamp << endl;//PC TriggerTimeStamp: 1594803786
      getline(*ifs,s); if(DEBUG) cout << s << endl;//ComputerTime Format 2: Wed Jul 15 11:03:06 2020
      getline(*ifs,s); if(DEBUG) cout << s << endl;//DC offset (DAC): 0x0CCC
//lets_pause();
    } 
    for(int i=0; i< Length; i++) 
    {
      getline(*ifs,s); if(DEBUG) cout <<i << " " << s<< endl;
      if(s=="") {return false; if(DEBUG) cout <<"File CUT, end run here" << endl;}
      ADCs.push_back(stoi(s)); if(DEBUG) cout <<" OK" << endl;
    }
...
//    cout << endl;
  ...
  bool LoadOscilloscope(TString file, bool DEBUG=false,int Length=52, int polarity=-1)
  {
    ifstream ifs;
    ifs.open(file);
    if (DEBUG){cout<<file<<endl;};
    TString Line1,Line2,Line3,Line4,Line5,Line6;
    ifs >> Line1; if(DEBUG) cout << "1 " << Line1 << endl;
    ifs >> Line2; if(DEBUG) cout << "2 " << Line2 << endl;
    ifs >> Line3; if(DEBUG) cout << "3 " << Line3 << endl;
    ifs >> Line4; if(DEBUG) cout << "4 " << Line4 << endl;
    ifs >> Line5; if(DEBUG) cout << "5 " << Line5 << endl;
    ifs >> Line6; if(DEBUG) cout << "6 " << Line6 << endl;
    for (int j=0;j<Length; j++)
    {
       double time,adc; char c;	  ifs >> time >> c >> adc;      
       if (polarity==1)ADCd.push_back(-adc);
       if (polarity==-1)ADCd.push_back(adc);
...
void Dump(std::vector<Waveform_t> *w, string file, bool Osci=false)
{
   ...
   t->Write("IR02");
   f.Close();
   cout << "Dumped to file " << file << endl << endl;
}
void Draw(std::vector<double> *w)
{
  TH1D *h = new TH1D("h","h",w->size(),0,w->size());
  for (int j=0; j<w->size(); j++) h->SetBinContent(j+1,w->at(j));
...
void Load(std::vector<string> file, std::vector<string> name)
{
  ...
  for(int i=0; i<file.size(); i++) ev.SetFile(file[i],name[i]);
  for(int j=0; j<100; j++) { ev.Draw(j); lets_pause();}
...
int ReadAndDump(string file, string outfile, bool TS=false)
{
  ...
  if(ifs.is_open())
  {
    ...
    std::vector<Waveform_t> wv; int i=0;
    bool debug=false;
    while(!ifs.eof())
    {
      Waveform_t w0; bool b;
...
int evt_Length(string file)
{
  ...
  ifstream fd1;
  fd1.open(file);
  fd1 >> Line1; cout << Line1<< endl;
  Line2.ReadToDelim(fd1,',');cout << Line2<< endl;
  Line2.ReadToDelim(fd1,',');cout << Line2<< endl;
  Line2.ReadToDelim(fd1,',');cout << Line2<< endl;
  Line2.ReadToDelim(fd1,',');cout << Line2<< endl;
  Line3=Line2.Data(); 
  Length=stoi(Line3); cout<<"Length: "<<Length<<endl;//lets_pause();
  fd1.close();
  return Length;
}

//************************* Funcion para leer binario del ADC DT5725 ******************
int ReadAndDumpBinary(string file, string outfile)
{
  ...
  while (1)
  {
    ...
    {   
      ...
      if (PrevEvent==filehd[3]){break;} else{PrevEvent=filehd[3];}; 
    ...
    if(wv.size()%20000==0){ cout << "Loaded waveform " << wv.size()<< endl;}
...
//************************* Funcion para leer .txt del osciloscopio ******************
int ReadAndDumpOscilloscope(string file, string outfile,int nevents=-1,int polarity=-1)
{
 int corrupted=0;
 cout << "Loading " << file+"00001.txt" << endl;
 ifstream ifs;
 int Length=evt_Length(file+"00001.txt"); cout << Length << endl;
 ifs.open(file+"00001.txt");
 if(ifs.is_open())
 {
    cout<<"File opened."<<endl; std::vector<Waveform_t> wv; int i=1;
    bool debug=false;
    while(ifs.is_open())
    {
      ifstream ifs2;
      string file2=Form("%s%05i.txt",file.c_str(),i); if(debug) cout << "Loading file " << file2 << endl;
	    TString Line1;
      ifs.close();
      ifs.open(file2);
	    ifs >> Line1;
	    if (!Line1.Contains("Waveform")){corrupted+=1;i++;continue;}
      if (!ifs.is_open()) continue;
      Waveform_t w0;
      bool myb=w0.LoadOscilloscope(file2,debug,Length,polarity);
      if(myb)
      {
        if (debug) {Draw(&(w0.ADCd));gPad->Update();};
        wv.push_back(w0);
        if (i==0) cout << "Wvf length: " << wv[0].ADCd.size() << " samples." << endl;
        i++;
      }
      if(i%5000==0){ cout << "Loaded waveform " << i << endl;}
    }
    cout << "Loaded " << wv.size() << " waveforms from " << file << " - Sampling: " << wv[0].GetSampling() << endl;
    cout << "Wvf length: " << wv[0].ADCd.size() << " samples." << endl;
    cout << "Run duration: " << 1.0*(wv[wv.size()-1].GetTriggerTimeStamp()-wv[0].GetTriggerTimeStamp())/60.0 << " min." << endl;
    cout << "Data Rate: " << 1.0*wv.size()/(wv[wv.size()-1].GetTriggerTimeStamp()-wv[0].GetTriggerTimeStamp()) << " Hz." << endl;
    cout << "PCTS Run duration: " << 1.0*(wv[wv.size()-1].GetPCTimeStamp()-wv[0].GetPCTimeStamp())/60.0 << " min." << endl;
    cout << "PCTS Data Rate: " << 1.0*wv.size()/(wv[wv.size()-1].GetPCTimeStamp()-wv[0].GetPCTimeStamp()) << " Hz." << endl;
    cout << "Corrupted files (with no Waveform on the first line)) "<<corrupted<<endl;
