namespace ana
{
   ...
   public:
      ...
      bool CutIsPedestalRange = false;
      bool CutIsPedestal = false;
      bool CutPeakTimeRangePro=false; double PeakTimeRangePro_minTime; double PeakTimeRangePro_maxTime; double SPEAmp;
      bool CutMaxAmpChargeRange=false; double MaxAmp; double MaxCharge;
      ...
      {
         CutIsPedestalRange=true;
      }
      void SetCutIsPedestal()
      {
         CutIsPedestal=true;
      }
      void SetCutPeakTimeRangePro(double minTimePro, double maxTimePro, double speAmp) //cut events with peak time out of the range and amp over noise
      {
         CutPeakTimeRangePro=true;
         PeakTimeRangePro_minTime=minTimePro;
         PeakTimeRangePro_maxTime=maxTimePro;
         SPEAmp=speAmp;
      }
      void SetCutMaxAmpChargeRange(double maxamp, double maxcharge)
      {
         CutMaxAmpChargeRange = true;
         MaxAmp = maxamp;
         MaxCharge = maxcharge;
      }
      ...
      void ApplyCuts(waveana::Waveform_t &myevt, int pm, TH1D *h = NULL)
      {
         ...
         if (CutIsPedestalRange)                   if (!myevt.getIsPedestalRange()) {myevt.setCut(true);}
         if (CutIsPedestal)                        if (!myevt.getIsPedestal()) {myevt.setCut(true);}
         if (CutPeakTimeRangePro)                  if ((myevt.getMaxAmplitudeRange()>SPEAmp)  && (myevt.getPeakTime()<PeakTimeRangePro_minTime || myevt.getPeakTime()>PeakTimeRangePro_maxTime)){ myevt.setCut(true);}
         if (CutMaxAmpChargeRange)                if ((myevt.getMaxAmplitudeRange()<MaxAmp)  && (myevt.getChargeRange()>MaxCharge)){ myevt.setCut(true);}
      }
